...
The aim of this study is to \DIFdelbegin \DIFdel{provide a }\DIFdelend \DIFaddbegin \DIFadd{determine whether delegation is useful in modern programming languages and whether it could replace a classical object inheritance model in existing software projects. This involves first providing a }\DIFaddend clearer understanding of the ways delegation and classical inheritance are used in real world software development projects across programming languages with varying native support for each. \DIFdelbegin \DIFdel{This is achieved by examining existing examples of software projects }\DIFdelend \DIFaddbegin \DIFadd{To achieve this, corpora of existing software projects are examined }\DIFaddend to produce empirical evidence of the frequency at which these two structures are used in typical \DIFdelbegin \DIFdel{programs}\DIFdelend \DIFaddbegin \DIFadd{software projects}\DIFaddend .
...
\DIFdel{To analyse each language, we first needed to collect a corpus representative of that language's use in real world software development projects.In the case of Java, we adopted The Qualitas Corpus, which is a large collection of open source projects written in the Java language~\mbox{%DIFAUXCMD
\cite{QualitasCorpus}
}%DIFAUXCMD
. Likewise, with JavaScript, we have adopted an existing corpus used by the team that developed JSClassFinder~\mbox{%DIFAUXCMD
\cite{JSClassFinder}
}%DIFAUXCMD
. }\DIFdelend \DIFaddbegin \DIFadd{The process for analysing a Java project follows a pipeline structure where each file is parsed and analysed in isolation. The resulting statistics of each file are then aggregated to form the overall statistics across the projects. This file isolation is important because the syntax trees produced by ANTLR consume large amounts of memory so it is not possible to hold all the Java files for a project in memory simultaneously.
}\DIFaddend \newline

\DIFdelbegin \DIFdel{For the other studied languages, Python, Lua , andScala, quality existing corpora could not be found }\DIFdelend \DIFaddbegin \begin{center}
...
}\DIFdelend \DIFaddbegin \DIFadd{This indicates that, in the median project in the JavaScript corpus, over a third of all functions are modelling some form of class or method behaviour. A value of this magnitude shows that the use of classical inheritance in JavaScript is highly prevalent despite the language's lack of native support for its implementation.
}
