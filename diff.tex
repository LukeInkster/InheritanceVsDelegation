...
%DIF ADD InheritanceVsDelegation/delegationvsinheritance.tex      Tue Oct 11 16:46:28 2016
              , a4paper
              , twoside
              , openright
              ]{report}
...
%DIF -------
\supervisors{Dr. Alex Potanin, Prof. James Noble \& Tim Jones} %DIF > 
%DIF -------
...
\DIFadd{Section 12.5 of the Java Language Specification~\mbox{%DIFAUXCMD
\cite{JavaLangSpec}
}%DIFAUXCMD
...
\DIFaddbegin \DIFadd{The paper }\textit{\DIFadd{Object Inheritance Without Classes~\mbox{%DIFAUXCMD
\cite{InheritanceWithoutClasses}
}%DIFAUXCMD
...
. Analysing code which was created after the addition of native class support could no longer be considered to be a representation of class usage in a language offering only delegation natively.
}
...
\section{\DIFdelbegin \DIFdel{Analysing Corpora}\DIFdelend \DIFaddbegin \DIFadd{C}\# \DIFadd{Analysis}\DIFaddend }
\DIFdelbegin \textit{\DIFdel{The Qualitas Corpus: A Curated Collection of Java Code for Empirical Studies~\mbox{%DIFAUXCMD
\cite{QualitasCorpus}
}%DIFAUXCMD
}} %DIFAUXCMD
\DIFdel{discusses many of the choices behind the construction of the Qualitas Corpus. A corpus is defined as }\textit{\DIFdel{"a collection of writings, conversations, speeches, etc., that people use to study and describe a language"}}%DIFAUXCMD
\DIFdel{. In the Qualitas Corpus, the collection is of projects written in the Java programming language. This paper explores the reasoning behind the choices which led to the structure of the corpus as it is. Notably, the paper clarifies that the Java language was chosen for a few specific reasons:
}%DIFDELCMD < \begin{itemize}
%DIFDELCMD < 	\item %%%
\DIFdel{Open source Java code is abundant and easy to find.
Much more so than }\DIFdelend \DIFaddbegin \label{CSharpLit}
\DIFadd{The interesting aspect of the }\DIFaddend C\# \DIFdelbegin \DIFdel{, and similarly to }\DIFdelend \DIFaddbegin \DIFadd{programming language for the purposes of this study is the mandatory inclusion of the }\cs{virtual} \DIFadd{keyword to allow for dynamic dispatch of a method to occur at runtime. In contrast, Java dispatches methods dynamically by default and requires the inclusion of a }\java{final} \DIFadd{modifier to declare a statically dispatched method. Due to optimisations in the JVM through which it can still inline any virtual method for which no class with an overriding method has been loaded through the class loader, developers who omit the }\java{final} \DIFadd{keyword will likely also avoid the performance penalty associated with dynamic dispatch~\mbox{%DIFAUXCMD
\cite{JavaInlining}
}%DIFAUXCMD
...
\DIFadd{As a result of the requirement of the }\cs{virtual} \DIFadd{keyword, }\DIFaddend C\DIFdelbegin \DIFdel{++. }%DIFDELCMD < \item %%%
\DIFdel{Java code tends to be relatively easier to parse and analyse than many other languages including C++ due to the simpler grammar of the language. }%DIFDELCMD < \end{itemize}
%DIFDELCMD < %%%
\DIFdel{The paper also justifies the choice of projects in the corpus as they are open source and provide a wide array of different usages of the language to help to ensure variation in the code. }\DIFdelend \DIFaddbegin \# \DIFadd{integrated developer environments are able to offer useful warnings about questionable practices involving the use of virtually dispatched methods. One of these warnings discourages developers from making calls to virtual methods from constructors of non-sealed class. This rule is outlined in Microsoft Developer Network's usage warning }\textit{\DIFadd{CA2214: Do not call overridable methods in constructors~\mbox{%DIFAUXCMD
...
\DIFaddbegin \DIFadd{As Lippert describes, }\textit{\DIFadd{``Calling methods on derived types from constructors is dirty pool, but it is not illegal~\mbox{%DIFAUXCMD
\cite{NoDowncalls}
}%DIFAUXCMD
...
\section{\DIFadd{Software Metrics}}
\DIFaddend \textit{Towards a Metrics Suite for Object Oriented Design~\cite{MetricsSuite}} includes a variety of useful terms for defining measurements of inheritance within programs written in object oriented languages. These include:
\begin{itemize}
...
\DIFaddbegin \section{\DIFadd{Analysing Corpora}}
\textit{\DIFadd{The Qualitas Corpus: A Curated Collection of Java Code for Empirical Studies~\mbox{%DIFAUXCMD
\cite{QualitasCorpus}
}%DIFAUXCMD
}} \DIFadd{discusses many of the choices behind the construction of the Qualitas Corpus. A corpus is defined as }\textit{\DIFadd{``a collection of writings, conversations, speeches, etc., that people use to study and describe a language"}}\DIFadd{. In the Qualitas Corpus, the collection is of projects written in the Java programming language. This paper explores the reasoning behind the choices which led to the structure of the corpus as it is. Notably, the paper clarifies that the Java language was chosen for a few specific reasons:
}\begin{itemize}
	\item \DIFadd{Open source Java code is abundant and easy to find. Much more so than C}\#\DIFadd{, and similarly to C++.
	}\item \DIFadd{Java code tends to be relatively easier to parse and analyse than many other languages including C++ due to the simpler grammar of the language.}\newline
\end{itemize}

\DIFadd{The paper also justifies the choice of projects in the corpus as they are open source and provide a wide array of different usages of the language to help to ensure variation in the code.
}\newline

\textit{\DIFadd{The Use and Limitations of Static-Analysis Tools to Improve Software Quality~\mbox{%DIFAUXCMD
...
To determine how developers are using delegation and inheritance, the first stage is to define \DIFdelbegin \DIFdel{methods to identifying each}\DIFdelend \DIFaddbegin \DIFadd{a clear way of identifying the behaviour of each pattern}\DIFaddend . This is achieved by defining code patterns which, when found in a project, indicate the use of delegation or inheritance. This chapter outlines the patterns which are representative of forwarding, delegation and \DIFdelbegin \DIFdel{uniform identity. }\DIFdelend \DIFaddbegin \DIFadd{Uniform Identity. Lastly, it will briefly cover a couple of object inheritance models which were not explored in this study.
}\DIFaddend 

\section{Forwarding}
Under a forwarding model\DIFdelbegin \DIFdel{of object inheritance}\DIFdelend , calls to \DIFdelbegin \DIFdel{this.f(...) }\DIFdelend \DIFaddbegin \java{this.f(...)} \DIFaddend are passed to some \DIFdelbegin \DIFdel{other.f(...)}\DIFdelend \DIFaddbegin \java{other.f(...)}\DIFaddend , transferring any necessary information as call parameters.
\newline
...
\section{Delegation}
Delegation \DIFdelbegin \DIFdel{can be seen as forwarding this.f(...) calls to other.f(...) }\DIFdelend \DIFaddbegin \DIFadd{is often described as forwarding }\java{this.f(...)} \DIFadd{calls to }\java{other.f(...)} \DIFaddend \textbf{on behalf of this.} That is, \DIFdelbegin \DIFdel{call other.f(...) }\DIFdelend \DIFaddbegin \DIFadd{dispatch the call to }\java{other.f(...)} \DIFaddend but have the self reference within that call \DIFdelbegin \DIFdel{set }\DIFdelend \DIFaddbegin \DIFadd{point back }\DIFaddend to my self reference. \DIFaddbegin \DIFadd{This is explained in further detail in Section \ref{inheritanceModels}.
}\DIFaddend \newline
...
. Under this model, the parent object is created first, along with its identity, then that parent object will be mutated by replacing its properties with those of the child object where overrides are declared. This model matches object construction as it occurs in C++.
 ...
 \newpage \DIFdelbegin \chapter{\DIFdel{Analysis}}%DIFAUXCMD
\addtocounter{chapter}{-1}%DIFAUXCMD
%DIFDELCMD < \label{C:analysis} 
%DIFDELCMD < %%%
\DIFdel{The core of this empirical study is the analysis of corpora of code written in each of the investigated languages. This analysis makes use of many static code analysis methods including the following:
}%DIFDELCMD < \begin{itemize}
%DIFDELCMD < 	\item %%%
\DIFdel{Grep is used to perform regular expression searches on files. This can detect the more simple patterns explored in this paper.
}%DIFDELCMD < \item %%%
\DIFdel{ANTLR is a tool which accepts a language grammar and a valid file in that language. From this it constructs a syntax tree to represent the file. }%DIFDELCMD < \item %%%
\DIFdel{JSClassFinder is a tool which detects class and method declarations in JavaScript code.
	}%DIFDELCMD < \item %%%
\DIFdel{Esprima accepts a JavaScript file as input and produces a JSON representation of the syntax tree of that file. This JSON file is then used as the input to JSClassFinder. }%DIFDELCMD < \end{itemize}
%DIFDELCMD < %%%
\DIFdel{Each of these tools helps to extract valuable information from one of more of the languages analysed in this study.
 }\DIFdelend \DIFaddbegin \chapter{\DIFadd{Methodology}}\label{C:method} 
\DIFaddend 

\section{Selecting Languages}
The languages \DIFdelbegin \DIFdel{which have been explored thus far are limited to Javaand JavaScript }\DIFdelend \DIFaddbegin \DIFadd{explored in this study include Java, C}\#\DIFadd{, JavaScript and Lua}\DIFaddend . These languages were \DIFdelbegin \DIFdel{covered first because they both have large }\DIFdelend \DIFaddbegin \DIFadd{were selected because they each have large enough }\DIFaddend open source communities \DIFdelbegin \DIFdel{and they differ on their native method of object inheritance which provides a good avenue for comparison. The other languages which will be explored in the remainder of the project are Python, Lua and Scala. These are included to ensure that a wide enough variety of languages are analysed that it is possible to make conclusions about the usage of delegation and inheritance across languages}\DIFdelend \DIFaddbegin \DIFadd{to gather meaningful corpora of projects and, between them, they offer a wide range of native object inheritance model implementations. They also provide a helpful division between two languages with native support for classical inheritance object models and two languages with native support for delegation object models.
}\newline
...
\DIFadd{JavaScript and Lua both offer delegation natively so can provide a measure of how often developers are making use of these delegation features. They also show how often developers in these languages are choosing to ignore the languages' native features in favour of classical inheritance models}\DIFaddend .
...
For \DIFdelbegin \DIFdel{the other studied languages, Python, Lua , and Scala}\DIFdelend \DIFaddbegin \DIFadd{Lua and C}\#\DIFaddend , quality existing corpora could not be found \DIFaddbegin \DIFadd{so we had to build our own}\DIFaddend . For each of these languages, the top 25 open source projects were sourced from GitHub's \DIFdelbegin \DIFdel{"}\DIFdelend \DIFaddbegin \DIFadd{``}\DIFaddend Trending this month" list \DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{as of }\DIFaddend June, 2016. This source was chosen because it provides a group of projects for each language which are in active development as measured by GitHub, and which are easy to access. This helps to ensure that the analysis performed will \DIFdelbegin \DIFdel{be as relevant as possible to modern software development}\DIFdelend \DIFaddbegin \DIFadd{contain repositories which are currently active so likely make use of newer language features.
}

\section{\DIFadd{Static Analysis Tools}}
\DIFadd{The core of this empirical study is the analysis of corpora of code written in each of the investigated languages. This analysis makes use of many static code analysis methods including the following:
}\begin{itemize}
	\item \texttt{\DIFadd{grep}} \DIFadd{is used to perform regular expression searches on files. This can detect some of the more simple patterns explored in this paper. More specifically, an implementation of }\texttt{\DIFadd{grep}} \DIFadd{known as PCREGrep (Perl Compatible Regular Expression }\texttt{\DIFadd{grep}}\DIFadd{) was used to allow multiline analysis which is a requirement of a source code corpus analysis.
	}\item \DIFadd{ANTLR (Another Tool For Language Recognition) is a tool which accepts a language grammar as input and produces a lexer and parser. This lexer and parser can then accept a file which conforms to the grammar definition and construct a syntax tree to represent that file.
	}\item \DIFadd{JSClassFinder is a tool which detects patterns indicative of class and method declarations in JavaScript projects. It accepts JSON representations of the syntax trees of JavaScript files as input and produces the Class Usage Ratio of the syntax tree as its output~\mbox{%DIFAUXCMD
\cite{JSClassFinder}
}%DIFAUXCMD
.
	}\item \DIFadd{Esprima accepts a JavaScript code file as input and produces a JSON representation of the syntax tree of that file as output. This JSON file can then be used as the input to the JSClassFinder tool.
}\end{itemize}

\section{\DIFadd{Java Analysis}}
\DIFadd{Finding occurrences of classical inheritance in Java is as simple as looking for the extends keyword with a }\texttt{\DIFadd{grep}} \DIFadd{regular expression search. Finding examples of delegation and forwarding is more difficult and requires more information about the syntax tree of the program. To achieve this, each program of the corpus was passed through ANTLR which parses each file according to a lexer and parser generated from a Java grammar. ANTLR then constructs an abstract syntax tree which can then be traversed to search for relevant patterns.
}\newline

\DIFadd{The process for analysing a Java project follows a pipeline structure where each file is parsed and analysed in isolation. The resulting statistics of each file are then aggregated to form the overall statistics across the projects. This file isolation is important because the syntax trees produced by ANTLR consume large amounts of memory so it is not possible to hold all the Java files for a project in memory simultaneously.
}\newline
...
\DIFadd{As with Java, C}\# \DIFadd{was analysed using a lexer and parser generated by loading a C}\# \DIFadd{6 grammar into ANTLR. The analysis for each project in the corpus was performed in three major passes:
	...
	\item \DIFadd{Use ANTLR, along with a C}\# \DIFadd{preprocessor grammar, to perform the first stage towards forming a syntax tree. This stage evaluates preprocessor directives in the program to ensure the remaining file can be transformed into a well formed syntax tree. Included in this stage is the removal of }\cs{\#region} \DIFadd{tags and conditional directives which exclude and include blocks of source code based on boolean arguments.
...
\DIFadd{As a result of the wide ranging methods of implementing classical inheritance in JavaScript, the effort required to do so accurately is great. Prior work in the field made the JavaScript analysis in this study possible, primarily the existence of tool JSClassFinder~\mbox{%DIFAUXCMD
\cite{JSClassFinder}
}%DIFAUXCMD
. The JavaScript analysis in this study consisted mainly of a recreation of the JSClassFinder study. JSClassFinder is a tool created by a team of researchers to analyse the extent to which JavaScript developers use classes in their projects}\DIFaddend .

\DIFaddbegin \section{\DIFadd{Lua Analysis}}
\DIFadd{The Lua corpus was analysed with }\texttt{\DIFadd{grep}} \DIFadd{to identify code patterns and keywords associated with class usage. There exists a variety of patterns used to implement classical inheritance in Lua as described by the Lua-Users Wiki~\mbox{%DIFAUXCMD
...
\DIFadd{The first of these patterns is the presence of }\code{identifier = setmetatable()} \DIFadd{in a Lua program. The }\code{setmetatable()} \DIFadd{function is the core of all suggested object orientation implementations so the detection of this pattern is vital. Unfortunately, while the use of this function is typically considered necessary for object orientation to exist in a Lua program, the pattern is often encapsulated in a function of a different name which makes the actual extent of object orientation usage more difficult to measure. In response to the practice of encapsulation of these patterns, this study has also included measures of the presence of two function names which are typically used to wrap these classical inheritance behaviours. These are the }\code{class()} \DIFadd{and }\code{new()} \DIFadd{functions.
}

\section{\DIFadd{Metrics}}

\subsection{\DIFadd{Java}}
\subsection{\DIFadd{C}\#}
\subsection{\DIFadd{JavaScript}}
\subsection{\DIFadd{Lua}}
...
\DIFdelbegin \DIFdel{The process for analysing a Java project follows a pipeline structure where each file is parsed and analysed in isolation. The resulting statistics of each file are then aggregated to form the overall statistics across the projects. This file isolation is important because the syntax trees produced by ANTLR consume large amounts of memory so it is not possible to hold all the Java files for a project in memory simultaneously.
}%DIFDELCMD < \newline
%DIFDELCMD < 

%DIFDELCMD < \captionof{figure}{Java Analysis Pipeline}
...
..)                                                                                                           }\DIFdelend \DIFaddbegin \DIFadd{9057  }&                    & \DIFadd{12.66\%            }\DIFaddend \\ \hline
		\DIFdelbegin \DIFdel{Inheritance 5 - Node. js        }\DIFdelend \DIFaddbegin \DIFadd{Methods                                                                                                        }\DIFaddend & \DIFdelbegin \DIFdel{util.
inherits(. ..)                                                                                                                         }\DIFdelend \DIFaddbegin \DIFadd{222121 }&                    &                    \DIFaddend \\ \hline
...
\DIFdelbegin %DIFDELCMD < \newline
%DIFDELCMD < %%%
\DIFdel{The }\DIFdelend \DIFaddbegin \end{center}
...
\section{\DIFadd{Distribution of Number of Subclasses}}
...
\DIFadd{The C}\# \DIFadd{subclassing distribution in \ref{cSharpDistribution} matches the distribution discovered through the investigation in }\textit{\DIFadd{Understanding the Shape of Java Software}}\DIFadd{~\mbox{%DIFAUXCMD
\cite{ShapeOfJava}
}%DIFAUXCMD
and reproduced in this study. The distribution found in the analysis of the Java corpus can be found in the following chart: 
}
...
\DIFadd{TODO - Possibly merged with results }\DIFaddend \newpage 
 \newpage \DIFdelbegin \chapter{\DIFdel{Conclusions}}%DIFAUXCMD
\addtocounter{chapter}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \chapter{\DIFadd{Conclusion}}\DIFaddend \label{C:con}
\DIFdelbegin \DIFdel{Some conclusions can be drawn from the data gathered thus far. This data covers the analyses of Java and JavaScript corpus data.
It was found that there was a reasonable amount of crossover }\DIFdelend \DIFaddbegin \DIFadd{We found that differences }\DIFaddend between the native support offered by each language and the patterns used by developers \DIFaddbegin \DIFadd{of projects in that language were fairly frequent}\DIFaddend :
\begin{itemize}
	...
	}\item \DIFaddend In JavaScript, where delegation is natively supported, \DIFdelbegin \DIFdel{36}\DIFdelend \DIFaddbegin \DIFadd{15}\DIFaddend \% of the functions in the median project were used to emulate class or method behaviour.
	\DIFaddbegin \item \DIFadd{In Lua, where delegation is natively supported, 17\% of all files contained patterns indicative of class behaviour.
}\DIFaddend \end{itemize}
From these numbers, it appears developers are more \DIFdelbegin \DIFdel{wiling to use }\DIFdelend \DIFaddbegin \DIFadd{willing to use classical }\DIFaddend inheritance structures, thus ignoring native delegation. \DIFdelbegin \DIFdel{Further research on other languages will help to clarify this claim. }\DIFdelend \DIFaddbegin \DIFadd{It is difficult to determine whether this is because classical inheritance is necessary for aspects of the projects or if its use is simply more common because developers find it more comfortable.
}\newline
...
\subsection{\DIFadd{Measuring Intent}}
\DIFadd{Ideally, an empirical study of this nature would come with measures of the precision and recall of the algorithm. For this study, these measures would be dependent on comparing the patterns detected by the algorithms against the intent of the developer. Unfortunately, there is no easy way to accurately differentiate between a developer's intent and the patterns they use in their code. This issue is discussed in }\textit{\DIFadd{Does JavaScript Software Embrace Classes?}} \DIFadd{which describes the creation of the JSClassFinder tool~\mbox{%DIFAUXCMD
\cite{JSClassFinder}
}%DIFAUXCMD
. Even when manually inspecting files and analysis results, it is often not possible to determine whether the result of the analysis is truly indicative of the developer's intent. For this reason, it is only possible to approximate the values of precision and recall based on some assumptions about whether certain patterns are, in fact, representative of the behaviours under analysis. Despite this, having estimated ranges of these precision and recall values could help to improve confidence in the research so would be valuable future work. }\DIFaddend \newpage 
